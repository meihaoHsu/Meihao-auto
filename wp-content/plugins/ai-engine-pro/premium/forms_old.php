...
define( 'MWAI_FORMS_FRONT_PARAMS', [ 'id', 'label', 'type', 'name', 'options',
  'required', 'placeholder', 'default', 'maxlength', 'rows' ] );
define( 'MWAI_FORMS_SERVER_PARAMS', [ ] );

class MeowPro_MWAI_Forms_Old {
  private $core = null;
    ...
    switch ( $type ) {
      case 'select':
        if ( !empty( $options ) ) {
          ...
          $html .= '<div class="mwai-form-field-container">';
  ...
  function chatgpt_style( $id ) {
    $css = file_get_contents( MWAI_PATH . '/premium/styles/forms_old_ChatGPT.css' );
